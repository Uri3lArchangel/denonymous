...
import {
  ...
  sendRelpy,
} from "../DB/queries/denonymous/query";
...
export const createDenonyous = async (topic: string, desc: string) => {
  try {
    ...
    const res = await createDenonymous(
      sessionToken.email,
      String(topic),
      String(desc)
    );
...
export const sendRelpyAction = async (
  username: string,
  key: string,
  reply: any
) => {
  ...
  try {
    ...
    for (let i = 0; i < r.length; i++) {
      for (let j = 0; j < r[i].media.length; j++) {
        let a = await fetch(
          process.env.baseURL + "/api/edgestore/delete-file",
          {
            method: "POST",
            credentials: "include",
            headers: {
...
export const changeResponsesVisibilityActiion = async (
  denonymousKey: string
) => {
  try {
    let cookie = cookies().get("denon_session_0");
    if (!cookie || !cookie.value) redirect("/auth/signin");
...
export const changeMediaSettingsAction = async (
