...
import { useModal } from "@/src/FE/components/libraries/Modals/ModalProvider";

function CreateDenonymousForm({
  handleModalClose,
  setWarning,
  setMessage,
  ...
  const {setDataValue,openModal}= useModal()
  const notification = useContext(NotificationContext)!;

  const topicRef = useRef<HTMLInputElement>(null);
  const descRef = useRef<HTMLTextAreaElement>(null);

  const createDenonymousFunction = async (
    e: React.MouseEvent<HTMLButtonElement>
  ) => {
    e.preventDefault();
    const topicError = document.getElementById(
      "topic_error"
    ) as HTMLParagraphElement;
    const descError = document.getElementById(
      "desc_error"
    ) as HTMLParagraphElement;
    ...
    if (!topic) {
      topicError.innerText = "Topic cannot be empty";
      return;
    }
    if (topic.length > 50) {
      topicError.innerText = "Topic max length exceeded";
      return;
    }
    if (desc.length > 100) {
      descError.innerText = "Description max length exceeded";
      return;
    }
    setPending(true);
    ...
    if (res.type == "warning" && res.message == "Max") {
      setMessage(
        "Maximum denonymous boxes allowed for free accounts, upgrade to premium for unlimited boxes"
      );
