...
export function MultiFileDropzoneUsage({
  ...
  return (
    <div className="full_response_container_textarea ">
        {warning?<WarningModal message={message} setModal={setWarning}  />:null}

      <div
        className="flex w-[60%] max-w-[300px]  md:h-fit justify-between mx-auto my-2 md:my-0 md:w-fit "
        id="uploadMediaIcons"
      >
        <label
          aria-disabled={sending || mediaLimit.image || !premium}
          htmlFor={`${sending || mediaLimit.image || !premium ? "" : "chick_a"}`}
        >
          <AiFillPicture
            disabled={sending || mediaLimit.image}
            size={45}
            className={`text-[#404040]  hover:text-[#ffdf00] md:mx-2 ${
              mediaLimit.image
        ...
        <label
          aria-disabled={sending || mediaLimit.video}
          htmlFor={`${sending || mediaLimit.video ? "" : "chick_b"}`}
        >
          <BsCameraVideoFill
        ...
        <label
          aria-disabled={sending || mediaLimit.audio}
          htmlFor={`${sending || mediaLimit.audio ? "" : "chick_c"}`}
        >
          <MdOutlineAudioFile
      ...
      <div id="previewMediaContainer">
        <MultiFileDropzone
          ...
          value={fileStates1}
          disabled={sending || mediaLimit.image || !premium}
          generalFileStateAction={fileStates}
          setGeneralFileStateAction={setFileStates}
          onChange={async(files) => {
            if (mediaLimit.image) return;
            if(!premium){
              setMessage("Only premium denonymous boxes can allow image upload, would you like to upgrade this denonymous box to receive images?")
              setWarning(true)
      ...
      >
        <button
          ...
          onClick={async (e) => {
            ...
            await Promise.all(
              fileStates.map(async (addedFileState) => {
                try {
                  if (
                    addedFileState.progress != "COMPLETE" &&
                    !uploadedKeys.includes(addedFileState.key)
                  ) {
          ...
          className={
            fileStates.length < 1
              ? "hidden "
              : `bg-green-500 block mx-auto w-[90%] h-[50px] mt-6 rounded-[10px] bg-transparent gradient_elements_text border-[#ffdf00] border-2`
          }
        >
          upload media
        </button>
        ...
        <button
          ...
          onClick={async (e) => {
            try {
              ...
              const sendRelpyAction = (
                await import("@/src/BE/serverActions/actions")
              ).sendRelpyAction;
