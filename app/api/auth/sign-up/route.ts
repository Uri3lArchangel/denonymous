...
try{  const { email, password, confirmPassword } = await req.json();
...
console.log(err)
  ...
  if(errorMessage.includes("duplicate key error") && errorMessage.includes("email")){
    return NextResponse.json({message:"This email is already registered, try signing in",data:null},{status:400})
  }
    return NextResponse.json({message:"internal error",data:null},{status:500})
}
