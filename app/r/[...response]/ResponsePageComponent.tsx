...
async function ResponsePageComponent({
  ...
  const key = key_;
  const [pollingData, setPollingData] = useState<userModelType | null>(null);
  const [pollingU1, setPollingU1] = useState<u1 | null>(null);
  ...
  const fetchData = async () => {
    try {
      ...
      await connectMongo();
      let u1Data = await UserSec.findOne({ username: all.username }) as u1;
      if (!u1Data) {
        u1Data = await UserSec.create({ points: 0, username });
      }
      setPollingU1(u1Data);
    } catch (error) {
  ...
  if (!all) {
    // Return loading state or initial data while polling
    const initialAll = (await fetchUser(username)) as userModelType;
    all = initialAll;
    
    await connectMongo();
    let initialU1 = await UserSec.findOne({ username: initialAll.username }) as u1;
    if (!initialU1) {
      initialU1 = await UserSec.create({ points: 0, username });
    }
    u1 = initialU1;
  }
